function [F,FO] =sparse_fusion3(A,B,D,overlap,epsilon,AO,BO)
%    SR
%    Input:
%    A - input image A
%    B - input image B
...
%    Update: fusion of original images based on their own data
...
for ii = 1:length(gridx)
    for jj = 1:length(gridy)
        ...
        mean_fO=mean1O;
        for cf = 1:3           
            if sum(abs(w1O(:,cf)))<sum(abs(w2O(:,cf)))
                wO(:,cf)=w2O(:,cf);
                mean_fO(cf)=mean2O(cf);
            end
        end
        % Update-end:
